...
async function runAggregate() {
    try {
        ...
        await Sale.create([
            ...
            { category: "Stationary", amount: 200, status: "demo-sale" },
            { category: "Stationary", amount: 350, status: "demo-sale" },
        ]);
        const aggregationResult = await Sale.aggregate([
          //$match filters the inconing documents first
            {$match: {status:"demo-sale"}},
            //$group: groups by a specfic condition
            {$group:{
                _id:"$category",
